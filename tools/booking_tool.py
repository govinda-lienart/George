# followup_tool.py - Updated to use booking data
...
- Thanks them personally by name for their interest
- Mentions their stay dates to make it relevant
- Presents the activities in an engaging, personalized way
- Uses a friendly, conversational tone as George the receptionist
- Highlights the best options for their group size
...
def handle_followup_response(user_input: str, session_state) -> str:
    ...
    if intent == "POSITIVE":
        ...
        # Get booking info from session state
        booking_info = session_state.get("latest_booking_info", {})

        try:
            # Use LLM to generate a personalized response with the activity info and booking details
            response = (activity_response_prompt | llm).invoke({
                "client_name": booking_info.get("client_name", "valued guest"),
                "check_in": booking_info.get("check_in", "your check-in"),
                "check_out": booking_info.get("check_out", "your check-out"),
                "room_type": booking_info.get("room_type", "your room"),
                "num_guests": booking_info.get("num_guests", "your group"),
